...
export default function AuthModal({ onClose, onSuccess }: AuthModalProps) {
  const { registerWithEmail, loginWithEmail, loginWithGoogle, authLoading, authError, clearAuthError } = useAuthStore();
  ...
  if (!isFirebaseConfigured) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.6)' }}>
        <div className="w-full max-w-sm rounded-3xl border p-6 text-center" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
